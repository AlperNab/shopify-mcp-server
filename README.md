...
**Domain:** `E-commerce / Shopify Ops` · **Suite:** `E-commerce Growth Suite` · **Accent:** `#a3e635`
...
**Shopify Mcp Server** is a standalone, browser-based AI workflow app for **E-commerce / Shopify Ops**. It turns structured inputs, uploaded files, and project-specific settings into reviewable outputs using a deterministic local engine plus optional local/cloud LLM enhancement.
...
| `store`<br>store | text | No | — | Affects customization: store. |
| `scopes`<br>scopes | text | No | — | Affects customization: scopes. |
| `allowed_actions`<br>allowed actions | text | No | — | Affects customization: allowed actions. |
| `approval_rules`<br>approval rules | textarea | No | — | Affects customization: approval rules. |
| `rate_limits`<br>rate limits | text | No | — | Affects customization: rate limits. |
| `product_fields`<br>product fields | text | No | — | Affects customization: product fields. |
| `risk_policy`<br>risk policy | text | No | — | Affects customization: risk policy. |
...
uvicorn app.main:app --host 127.0.0.1 --port 9153
```
...
| `AI_SUITE_PORT` | Port for this project GUI, default `9153`. |
...
- Export download
- Job history
- Shopify Admin API required for live store actions
...
```bash
curl http://127.0.0.1:9153/api/health
```
...
```bash
curl -X POST http://127.0.0.1:9153/api/run \
...
```bash
AI_SUITE_HOST=0.0.0.0 AI_SUITE_PORT=9153 uvicorn app.main:app --host 0.0.0.0 --port 9153
```
...
- `plugins/shopify-mcp-server.json` for plugin metadata.
